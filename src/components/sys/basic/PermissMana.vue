  ...
  methods: {
    doDeleteRole(role) {
      this.$confirm(
        ...
        .then(() => {
          this.deleteRequest(
            "/system/basic/permission/deleteRole/" + role.id
          ).then(resp => {
            if (resp) {
              this.initRoles();
            }
    ...
    doAddRole() {
      if (this.role.name && this.role.nameZh) {
        this.postRequest("/system/basic/permission/addRole", this.role).then(
          resp => {
            if (resp) {
              this.initRoles();
              this.role.name = "";
    ...
    initSelectedMenus(rid) {
      this.getRequest("/system/basic/permission/getMidByRid/" + rid).then(
        resp => {
          if (resp) {
    ...
    initAllMenus() {
      this.getRequest("/system/basic/permission/getAllMenus").then(resp => {
        if (resp) {
          this.allMenus = resp;
        }
      });
    ...
    initRoles() {
      this.getRequest("/system/basic/permission/getAllRoles").then(resp => {
        if (resp) {
          this.roles = resp;
        }
      });
