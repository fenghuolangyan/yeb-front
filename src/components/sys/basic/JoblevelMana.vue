...
export default {
  ...
  methods: {
    deleteMany() {
      this.$confirm(
        ...
        .then(() => {
          ...
          this.deleteRequest(
            "/system/cfg/joblevel/batchDeletejoblevels" + ids
          ).then(resp => {
            if (resp) {
              this.initJls();
            }
    ...
    doUpdate() {
      this.putRequest(
        "/system/cfg/joblevel/updatejoblevel",
        this.updateJl
      ).then(resp => {
        if (resp) {
          this.initJls();
    ...
    deleteHandler(data) {
      this.$confirm(
        ...
        .then(() => {
          this.deleteRequest(
            "/system/cfg/joblevel/deletejoblevel/" + data.id
          ).then(resp => {
            if (resp) {
              this.initJls();
            }
    ...
    addJobLevel() {
      if (this.jl.name && this.jl.titleLevel) {
        this.postRequest("/system/cfg/joblevel/addjoblevel", this.jl).then(
          resp => {
            if (resp) {
              this.initJls();
            }
    ...
    initJls(page = 1) {
      this.page = page;
    //   debugger
      this.getRequest("/system/cfg/joblevel/getAlljoblevels/"+this.page+"/"+this.limit).then(resp => {
        if (resp) {
          this.total = resp.obj.total;
          this.jls = resp.obj.records;
          this.jl.name = "";
          this.jl.titleLevel = "";
        }
      });
